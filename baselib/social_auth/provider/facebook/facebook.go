...
// This package can be used as a reference implementation of an OAuth2 provider for social_auth.
package facebook
...
func New(clientKey, secret, callbackURL string, scopes ...string) *Provider {
	...
	p.Fields = strings.Join(scopes, ",")
	return p
}

// Provider is the implementation of `social_auth.Provider` for accessing Facebook.
...
func (p *Provider) InstallOAuth(conf *social_auth.ProviderConfig) {
	...
	*p = provider
	p.Fields = strings.Join(conf.Scopes, ",")
...
func userFromReader(reader io.Reader, user *social_auth.User) error {
	u := struct {
		ID        string `json:"id"`
