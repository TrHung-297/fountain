...
func TestFuncPanicRecovery(t *testing.T) {
	...
	if !strings.Contains(buf.String(), "YOLO") {
		t.Errorf("expected a panic to be logged, got none")
	}
}
...
func TestSnapshotEntries(t *testing.T) {
	...
	select {
	case <-time.After(OneSecond):
		t.Errorf("expected job runs at 2 second mark")
	case <-wait(wg):
	}
}
...
func TestMultipleEntries(t *testing.T) {
	...
	select {
	case <-time.After(OneSecond):
		t.Errorf("expected job run in proper order")
	case <-wait(wg):
	}
}
...
func TestRunningJobTwice(t *testing.T) {
	...
	case <-time.After(2 * OneSecond):
		t.Errorf("expected job fires 2 times")
	case <-wait(wg):
	}
}
...
func TestBlockingRun(t *testing.T) {
	...
	select {
	case <-time.After(OneSecond):
		t.Errorf("expected job fires")
	case <-unblockChan:
		t.Errorf("expected that Run() blocks")
	case <-wait(wg):
	}
}
...
func TestStartNoop(t *testing.T) {
	...
	case <-tickChan:
		t.Errorf("expected job fires exactly twice")
	}
}
...
func TestJob(t *testing.T) {
	...
	if actualName := cron.Entry(job2).Job.(testJob).name; actualName != "job2" {
		t.Errorf("wrong job retrieved: %s", actualName)
	}
...
func TestJobWithZeroTimeDoesNotRun(t *testing.T) {
	...
	var calls int64
	cron.AddFunc("* * * * * *", func() { atomic.AddInt64(&calls, 1) })
	cron.Schedule(new(ZeroSchedule), FuncJob(func() { t.Errorf("expected zero task will not run") }))
	cron.Start()
	defer cron.Stop()
	<-time.After(OneSecond)
	if atomic.LoadInt64(&calls) != 1 {
...
func TestStopAndWait(t *testing.T) {
	t.Run("nothing running, returns immediately", func(t *testing.T) {
		...
		case <-ctx.Done():
		case <-time.After(time.Millisecond):
			t.Errorf("context was not done immediately")
		}
	})
	...
	t.Run("a couple fast jobs and a slow job added, waits for slow job", func(t *testing.T) {
		...
		select {
		case <-ctx.Done():
			t.Errorf("context was done too quickly immediately")
		...
		case <-time.After(1500 * time.Millisecond):
			t.Errorf("context not done after job should have completed")
		}
	})
	...
	t.Run("repeated calls to stop, waiting for completion and after", func(t *testing.T) {
		...
		case <-ctx.Done():
			t.Errorf("context was done too quickly immediately")
		case <-ctx2.Done():
			t.Errorf("context2 was done too quickly immediately")
		case <-time.After(1500 * time.Millisecond):
		...
		case <-ctx2.Done():
			// expected
		case <-time.After(time.Millisecond):
			t.Errorf("context2 not done even though context1 is")
		}
		...
		case <-ctx3.Done():
			// expected
		case <-time.After(time.Millisecond):
			t.Errorf("context not done even when cron Stop is completed")
		}
