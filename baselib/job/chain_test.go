...
func TestChain(t *testing.T) {
	...
	if !reflect.DeepEqual(nums, []int{1, 2, 3, 4}) {
		t.Errorf("unexpected order of calls: %+v", nums)
	}
}
...
func TestChainDelayIfStillRunning(t *testing.T) {
	...
	t.Run("second run delayed if first not done", func(t *testing.T) {
		...
		if started != 1 || done != 0 {
			t.Errorf("expected first job started, but not finished, got: %d, %d", started, done)
		}
		...
		started, done = j.Started(), j.Done()
		if started != 2 || done != 2 {
			t.Errorf("expected both jobs done, got: %d, %d", started, done)
		}
	})
...
func TestChainSkipIfStillRunning(t *testing.T) {
	...
	t.Run("second run skipped if first not done", func(t *testing.T) {
		...
		started, done = j.Started(), j.Done()
		if started != 1 || done != 1 {
			t.Errorf("expected second job skipped, got: %d, %d", started, done)
		}
	})
	...
	t.Run("skip 10 jobs on rapid fire", func(t *testing.T) {
		...
		if done != 1 {
			t.Errorf("expected 1 jobs executed, 10 jobs dropped, got: %d", done)
		}
	})
