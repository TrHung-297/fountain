...
func TestAES256IGECryptorBase64(t *testing.T) {
	key := []byte("11111111111111111111111111111111")
	iv := []byte("1111111111111111")
	...
	Encrypted, err1 := cryptor.EncryptSimpleWithBase64(MyStringByte)
	if err1 != nil {
		fmt.Println(err1)
	}
	fmt.Printf("We encrypted the string this way: %s \n", Encrypted)

	Decrypted, err2 := cryptor.DecryptSimpleWithBase64(string(Encrypted))
	if err2 != nil {
		fmt.Println(err1)
	}
