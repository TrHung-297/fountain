...
/*
   The gtv plus client uses the public key certificate in the pkcs1 format, but the GO language does not find operations that support the PKCS8 format.
   reference
...
   Use the following method to generate:
...
func TestNewRSACryptor(t *testing.T) {
	...
	rsa := NewRSACryptor([]byte("test_pkcs1.key"))
